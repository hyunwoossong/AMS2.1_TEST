...
public class CommonUtil {
	...
	DateFormat currentTime = new SimpleDateFormat("YYYYMMddHHmmsss");
	Date now = null;
...
//		bw.write(new String(sLog.getBytes("euc-kr"),"ksc5601"));
	...
	public static String nullChknotrim(Object str_value) {
	...
	/*
	 * History : 2004/08/11, fire73, Created Version : 1.0 Comment : session이
	 * 유효한지 검사 -------------------------------------------------- obj_Src :
	 * session --------------------------------------------------
	 */
/*	public static boolean isSessionConnected(HttpSession ssion) {
		if (ssion.getAttribute("SVCNO") != null)
			return true;
		else
			return false;
	...
	/*public static Properties getReqValues(HttpServletRequest request,
			Properties props,String req_name) throws Exception {
		try {
			String tmp_req =  (String) request.getParameter(req_name);
			if(tmp_req == null || tmp_req.equals("")){

			}else{
				props.put(req_name		, tmp_req);
			}
		} catch (Exception ex) {
			System.out.println("CommonUtil.getReqValues(HttpServletRequest request,Properties props,String req_name) Error");
		}
		return props;
...
//				System.out.println("구분자"+i+" : "+(String)ht.get("file_gubn"+i));
     ...
     *
     *  <pre>
     *  1. 처리내용 : 화면의 ajax 요청에대한 결과값 세팅위한 ajaxResponse
     *
     */
	...
	// File Save Method
	////////////////////////////////////////////////
	public void makeTxt(String src, String text){
		try {
	 ...
	 public boolean savefile(String src, String text) {
	...
	public String getCurrentTime(String kt_msg, Exchange exchange){

		now = new Date();
		String filename = "FILECHECK_"+currentTime.format(now)+".txt";
	 ...
	 *
	 */
	public static String getGMTtime(String type){

		DateFormat datetype = new SimpleDateFormat("yyMMdd");
		DateFormat hourtype = new SimpleDateFormat("HHmm");
		DateFormat typebtype = new SimpleDateFormat("ddHHmm");
		...
		try{
			...
			if(type.equals("DATE")||type.equals("date")){
				datetime = datetype.format(gmtTime);
			}else if(type.equals("TIME")||type.equals("time")){
				datetime = hourtype.format(gmtTime);
			}else if(type.equals("TYPEB")||type.equals("typeb")){
				datetime = typebtype.format(gmtTime);
			}else{
				datetime = "000000";
			}
	...
	public static String getYearformat(){
		...
		if(MMFormat.format(now).equals("01")){
			mm = "A";
		}else if(MMFormat.format(now).equals("02")){
			mm = "B";
		}else if(MMFormat.format(now).equals("03")){
			mm = "C";
		}else if(MMFormat.format(now).equals("04")){
			mm = "D";
		}else if(MMFormat.format(now).equals("05")){
			mm = "E";
		}else if(MMFormat.format(now).equals("06")){
			mm = "F";
		}else if(MMFormat.format(now).equals("07")){
			mm = "G";
		}else if(MMFormat.format(now).equals("08")){
			mm = "H";
		}else if(MMFormat.format(now).equals("09")){
			mm = "I";
		}else if(MMFormat.format(now).equals("10")){
			mm = "J";
		}else if(MMFormat.format(now).equals("11")){
			mm = "K";
		}else if(MMFormat.format(now).equals("12")){
			mm = "L";
		}else{
	...
	public String randomchar(){
		DateFormat Date = new SimpleDateFormat("YYYYMMddsss");
		Date now = new Date();
		// RANDOM VALUE CREATE
		String random="";

		for (int i = 1; i <= 5; i++) {
			int in = (int) ((Math.random() * 26) + 65);
			random = random+in;
		    }
