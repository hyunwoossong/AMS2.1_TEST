...
import com.trx.validate.unipassAPI;
...
public class IkamsValidationDoc {
	...
	final static String IKAMS_STEP = "600";
	...
	// /////////////////////////////////////////////
	public  HashMap<String, Object> ikamsvalidationDoc(@Header("AMS_TRX_ROUTE") String ams_out_reverse_key, @Body String msg, Exchange exchange) throws Exception{
		...
		try{
			...
			logger.info(" === IKAMS Validation Check START === ");
...
//					    		}
				    		...
				    		fhlmap.put("OCI_LINE_CHK_I", oci.toString());
...
//					    		 ||oci_str.indexOf("/NFY/")  != -1
		    				...
		    				if(fhlbody.indexOf("OCI/KR/EXP/I/") !=-1
		    					|| fhlbody.indexOf("OCI/KR/IMP/I/") !=-1
		    					||fhlbody.indexOf("OCI/KR/EXP/X/") !=-1){
					...
					if(fhlbody.indexOf("SHP/") !=-1){
						...
						if(fhlbody.indexOf("CNE/") == -1){
							...
							if(CommonUtil.nullChk(fhlmap.get("IKAMS_ERRMSG"),"").equals("")){
								fhlmap.put("IKAMS_ERRMSG", "CNE LINE IS NULL");
								fhlmap.put("IKAMS_ERRMSG_KOR", "SHP 정보 입력 시 CNE 라인 필수");
							}
							flag = false;
							fhlmap.put("FLAG", flag);
						}
					...
					if(fhlmap.get("FLAG") == null){
						fhlmap.put("FLAG", flag);
					}
			}

			if(!(Boolean)fhlmap.get("FLAG")){

				if(CommonUtil.nullChk(exchange.getIn().getHeader("Pima_Check_Flag")).equals("false")){

					HashMap logMap = new HashMap();
					logMap.put("HISTORY_STATUS", "IKAMS Validation Check Error");
					logMap.put("AMS_TRX_ROUTE", ams_out_reverse_key);
					logMap.put("IKAMS_STEP", "301");
					...
					if(CommonUtil.nullChk(ocishpline) != null && CommonUtil.nullChk(ocishpline).length() > 0
						...
						if(CommonUtil.nullChk(fhlmap.get("OCI_SHP_T_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_SHP_T_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_SHP_KC_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_SHP_KC_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_SHP_U_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_SHP_U_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_SHP_E_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_SHP_E_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_CNE_T_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_CNE_T_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_CNE_KC_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_CNE_KC_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_CNE_U_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_CNE_U_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_CNE_E_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_CNE_E_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_NFY_T_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_NFY_T_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_NFY_KC_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_NFY_KC_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_NFY_U_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_NFY_U_FLAG")).length() > 0
								|| CommonUtil.nullChk(fhlmap.get("OCI_NFY_E_FLAG")) != null && CommonUtil.nullChk(fhlmap.get("OCI_NFY_E_FLAG")).length() > 0) {

							commonSql.insertOCICN(ams_out_reverse_key, logMap, fhlmap);
						}
					}
					flag = false;
					logger.info(" === IKAMS Validation Check Error === ");
				}else if(CommonUtil.nullChk(exchange.getIn().getHeader("Air_Check_Flag")).equals("false")){

					HashMap logMap = new HashMap();
					logMap.put("HISTORY_STATUS", "IKAMS Validation Check Error");
					logMap.put("AMS_TRX_ROUTE", ams_out_reverse_key);
					logMap.put("IKAMS_STEP", "401");
					commonSql.insertHistory(logMap);
					commonSql.updateTrace(ams_out_reverse_key, logMap, fhlmap);
					commonSql.insertFHL(ams_out_reverse_key, logMap, fhlmap);
					...
					fhlmap.put("KAMS_ACK", fnaMsg); //AMS ACK로 변경
					//EDI HISTORY INSERT[S]
					HashMap logMap = new HashMap();
					logMap.put("HISTORY_STATUS", "IKAMS Validation Check Error");
					logMap.put("AMS_TRX_ROUTE", ams_out_reverse_key);
					logMap.put("IKAMS_STEP", "609");
					commonSql.insertHistory(logMap);
					commonSql.updateTrace(ams_out_reverse_key, logMap, fhlmap);
					commonSql.insertFHL(ams_out_reverse_key, logMap, fhlmap);
				...
				//EDI HISTORY INSERT[S]
				HashMap logMap = new HashMap();
				logMap.put("HISTORY_STATUS", "IKAMS Validation Check");
				logMap.put("AMS_TRX_ROUTE", ams_out_reverse_key);
				...
				commonSql.insertHistory(logMap);
				commonSql.updateTrace(ams_out_reverse_key, logMap, fhlmap);
				//20160115 관세청 4세대 FHL 테이블
				if(CommonUtil.nullChk(fhlmap.get("OCI_FLAG")).equals("I") || CommonUtil.nullChk(fhlmap.get("OCI_FLAG")).equals("X")){
				...
				flag = true;
				logger.info(" === IKAMS Validation Check Success === ");
			}

			exchange.getIn().setHeader("EDI_PARSE", fhlmap);
			exchange.getIn().setHeader("IKAMS_Validation_Check_Flag", flag);
		} catch(IndexOutOfBoundsException e){
			logger.info(" === IKAMS Validation Check IndexOutOfBoundsException Error === ");
			logger.error("ERROR==>"+e.toString(), e);
			flag = false;
			mainExceptionManager.process(ams_out_reverse_key, "AMS-MAIN","IKAMS Validation Check IndexOutOfBoundsException",e, msg);
		}catch(NullPointerException e){
			logger.info(" === IKAMS Validation Check NullPointer Error === ");
			logger.error("ERROR==>"+e.toString(), e);
			flag = false;
			mainExceptionManager.process(ams_out_reverse_key, "AMS-MAIN","IKAMS Validation Check NullPointerException",e, msg);
		}catch(SQLException e){
			logger.info(" === IKAMS Validation Check SQL Error === ");
			logger.error("ERROR==>"+e.toString(), e);
			flag = false;
			mainExceptionManager.process(ams_out_reverse_key, "AMS-MAIN","IKAMS Validation Check SQLException",e, msg);
		}catch(Exception e){
			logger.info(" === IKAMS Validation Check Error === ");
	    	logger.error("ERROR==>"+e.toString(), e);
	    	String errorCode = new String();
	    	String IKAMS_ERRMSG = new String();
	    	if (CommonUtil.isExit(e.toString(), ":::")) {
				errorCode = e.toString().substring(e.toString().indexOf(":::")-4, e.toString().indexOf(":::"));
				IKAMS_ERRMSG = e.toString().substring(e.toString().indexOf(":::")+3);
			}else {
				errorCode = "9999";
				IKAMS_ERRMSG = e.toString();
			}
	    	IKAMS_ERRMSG = CommonUtil.nullChk(IKAMS_ERRMSG,"");
	    	fhlmap.put("ERRORCODE", errorCode);
	    	fhlmap.put("IKAMS_ERRMSG", (IKAMS_ERRMSG.length()>65)?IKAMS_ERRMSG.substring(0,65):IKAMS_ERRMSG);
	    	flag = false;
	    	mainExceptionManager.process(ams_out_reverse_key, "AMS-MAIN","IKAMS Validation Check Exception",e, msg);
	    }finally{
	    	exchange.getIn().setHeader("CHECK_FLAG",Boolean.valueOf(flag));
	    	//TRACE TABLE INSERT[E]
			logger.info(" === IKAMS Validation Check End ===");
			try{
		...
		try{
			...
			if(mbiline.length==3){
				if(Pattern.matches("[0-9]{3}\\-[0-9]{8}[A-Z]{6}",mbiline[1])){
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "MAWB NO OR ORG DST ERROR");
						map.put("IKAMS_ERRMSG_KOR", "MAWB 번호 또는 출/도착지 코드가 잘못 입력 되었습니다");
					}
					flag = false;
				}

				//관세청 4세대 적용 MAWB 개수 10자리 증가
				//중량 -> 자리수 변경(n[..7]p->n[..17]p)(정수(13) + 소수점 + 소수이하(3))
				if(Pattern.matches("[T]{1}[0-9]{1,10}[KL]{1}[0-9]{1,13}([.]{1}[0-9]{1,3})?",mbiline[2].trim())){
					if(mbiline[2].indexOf("K")!=-1){
						String[] PC_WT = mbiline[2].split("K");
						map.put("M_PC_CODE", "T");
						map.put("M_PC", Double.parseDouble(PC_WT[0].substring(1)));
						map.put("M_WT_CODE", "K");
						map.put("M_WT", String.valueOf(BigDecimal.valueOf(Double.parseDouble(PC_WT[1]))));
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "MAWB PC OR WT ITEM ERROR");
						map.put("IKAMS_ERRMSG_KOR", "MAWB 수량 또는 중량이 잘못 입력 되었습니다");
					}
					flag = false;
				}
				...
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "MAWB LINE FORMAT ERROR");
					map.put("IKAMS_ERRMSG_KOR", "MAWB 정보 입력 오류");
				}
				flag = false;
			}
			...
			ee.printStackTrace();
			map.put("IKAMS_ERRMSG", "FHL MBI FORMAT ERROR");
			flag = false;
		}
	...
	public boolean mawbCheckValue(HashMap<String, Object> map) {
		...
		boolean flag = true;
		try{
			if(Double.parseDouble(map.get("M_WT").toString())<=0){
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "MAWB WEIGHT INSERT ERROR");
					map.put("IKAMS_ERRMSG_KOR", "총 중량이 0 또는 작은 값은 입력 할 수 없습니다");
				}
				flag = false;
			}else if(Double.parseDouble(map.get("M_PC").toString())<=0){
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "MAWB PIECES INSERT ERROR");
					map.put("IKAMS_ERRMSG_KOR", "총 수량이 0 또는 작은 값은 입력 할 수 없습니다");
				}
				flag = false;
			}
		}catch(Exception e){
			e.printStackTrace();
			if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
				map.put("IKAMS_ERRMSG", "FHL CHECK VALUE EXCEPTION");
				map.put("IKAMS_ERRMSG_KOR", "");
			}
			flag = false;
		}
		return flag;
	...
	public boolean HBSCheck(HashMap<String, Object> map) {
		...
		try{
			...
			if(hbsline.length >= 7&&CommonUtil.nullChk(map.get("VERSION"),"").equals("4")){
				//관세청 4세대 관련 HWAB NO 20자리 증가
				if(Pattern.matches("[0-9A-Z]{1,20}", hbsline[1])){
						map.put("HBS_NO", hbsline[1].trim());
				}else{
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "HAWB NO ERROR");
						map.put("IKAMS_ERRMSG_KOR", "HAWB NO 입력 오류-" + hbsline[1].trim());
					}
					flag = false;
				}
				...
				if (Pattern.matches("[A-Z]{6}", hbsline[2])) {
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "HAWB ORG OR DST ERROR");
						map.put("IKAMS_ERRMSG_KOR", "HAWB 출발지 또는 도착지 코드가 잘못 입력 되었습니다");
					}
					flag = false;
				}
				// HAWB PIECES CHECK
				//관세청 4세대 개수 4 -> 10자리 증가
				if (Pattern.matches("[0-9]{1,10}", hbsline[3])) {
					map.put("H_PC", hbsline[3]);
				}else{
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "HAWB PIECES ERROR");
						map.put("IKAMS_ERRMSG_KOR", "HAWB 수량 입력 오류 - " + hbsline[3]);
					}
					flag = false;
				}
				// HAWB WEIGHT CHECK
				//관세청 4세대 중량 7 -> -> 자리수 변경(n[..7]p->n[..17]p)(정수(13) + 소수점 + 소수이하(3))
				if (Pattern.matches("[KL]{1}[0-9]{1,13}([.]{1}[0-9]{1,3})?", hbsline[4])) {
					map.put("H_WT_CODE", hbsline[4].substring(0,1));
					map.put("H_WT",	String.valueOf(BigDecimal.valueOf(Double.parseDouble(hbsline[4].substring(1)))));
				}else{
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "HAWB WEIGHT ERROR");
						map.put("IKAMS_ERRMSG_KOR", "HAWB 중량 입력 오류 - " + hbsline[4]);
					}
					flag = false;
				}
				// HAWB SLAC CHECK
				if (Pattern.matches("[0-9]{0,5}", hbsline[5])) {
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "HAWB SLAC ERROR");
						map.put("IKAMS_ERRMSG_KOR", "INNER PICE 입력 오류");
					}
					flag = false;
				}
				...
				//hawb goods
				 if (Pattern.matches("[A-Z0-9\\s\\-\\.]{1,15}", hbsline[6])) {
					 if(hbsline[6].replace(" ", "").length() == 0){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("H_COMMODITY", hbsline[6]);
							map.put("IKAMS_ERRMSG", "HAWB COMMODITY INSERT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "형식이 맞지 않는 품목이 입력 되었습니다." + hbsline[6]);
						}
						map.put("H_COMMODITY", hbsline[6]);
						flag = false;
					}else{
						...
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "HAWB COMMODITY ERROR");
							map.put("IKAMS_ERRMSG_KOR", "HAWB 품목 입력 오류");
						}
						flag = false;
					}

				if(hbs.length==2){
					//관세청 4세대 EAT 수출 항목일 경우 미기재시 NO로 신고 됨
					if(!Pattern.matches("((/[A-Z]{3}){1,9})", hbs[1])){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "HAWB SPECIAL HANDLING CODE ERROR");
							map.put("IKAMS_ERRMSG_KOR", "HAWB 스페셜 핸들링 코드 오류");
						}
						flag = false;
					}else{
				...
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL HBS FORMAT ERROR");
					map.put("IKAMS_ERRMSG_KOR", "HAWB 정보 입력 오류");
				}
				flag = false;
			}
	...
	public boolean hawbCheckValue(HashMap<String, Object> map) {
		...
		boolean flag = true;
		try{
			if(Double.parseDouble(map.get("H_PC").toString())<=0){
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "HWB PIECES INSERT ERROR");
					map.put("IKAMS_ERRMSG_KOR", "HWB 수량이 0 또는 작은 값은 입력 할 수 없습니다");
				}
				flag = false;
			}else if(Double.parseDouble(map.get("H_WT").toString())<=0){
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "HWB WEIGHT INSERT ERROR");
					map.put("IKAMS_ERRMSG_KOR", "HWB 중량이 0 또는 작은 값은 입력 할 수 없습니다");
				}
				flag = false;
			}
		}catch(Exception e){
	...
	public boolean TXTCheck(HashMap<String, Object> map) {
		...
		if(txt_line.length>=1 && txt_line.length<=19){
			String fltline = "";
			String dcsline = "";
			String snmline = "";
			String sccline = "";
			...
			for(int a=0; a<txt_line.length; a++){
				if(txt_line[a].startsWith("TXT/.COM.")){
					if (Pattern.matches("[TXT]{3}\\/.[COM]{3}\\.[A-Z0-9\\s\\-\\.]{1,60}", txt_line[a])) {
						...
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "FHL COM LINE");
							map.put("IKAMS_ERRMSG_KOR", "품목 상세정보 입력 오류1");
						}
						flag = false;
					}
				...
				if (a == 0) {
					...
					map.put("T_FREETEXT9", txt_line[a].replace("TXT/", "").replace("/", ""));
				}else if(a == 10){
					map.put("T_FREETEXT10", txt_line[a].replace("TXT/", "").replace("/", ""));
				}else if(a == 11){
					map.put("T_FREETEXT11", txt_line[a].replace("TXT/", "").replace("/", ""));
				}else if(a == 12){
					map.put("T_FREETEXT12", txt_line[a].replace("TXT/", "").replace("/", ""));
				}else if(a == 13){
					map.put("T_FREETEXT13", txt_line[a].replace("TXT/", "").replace("/", ""));
				}else if(a == 14){
					map.put("T_FREETEXT14", txt_line[a].replace("TXT/", "").replace("/", ""));
				}else if(a == 15){
					map.put("T_FREETEXT15", txt_line[a].replace("TXT/", "").replace("/", ""));
				}else if(a == 16){
					map.put("T_FREETEXT16", txt_line[a].replace("TXT/", "").replace("/", ""));
				}else if(a == 17){
					map.put("T_FREETEXT17", txt_line[a].replace("TXT/", "").replace("/", ""));
				}else if(a == 18){
					map.put("T_FREETEXT18", txt_line[a].replace("TXT/", "").replace("/", ""));
				}
			}
			...
			try{
				...
				if (Pattern.matches("[TXT]{3}\\/.[COM]{3}\\.[A-Z0-9\\s\\-\\.]{1,200}", com2)) {
					...
					if(Comodity.replace(" ", "").length() == 0){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "COMMODITY INSERT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "형식이 맞지 않는 품목 상세정보가 입력 되었습니다");
						}
						flag = false;
					}
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "FHL COM LINE");
						map.put("IKAMS_ERRMSG_KOR", "품목 상세정보 입력 오류3");
					}
				...
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL COM LINE");
					map.put("IKAMS_ERRMSG_KOR", "품목 상세정보 입력 오류4");
				}
				flag = false;
			}
			try{
				...
				if(!fltline.equals("")){
					...
					if(Pattern.matches("[/.FLT.]{6}[0-9]{0,8}\\-[A-Z0-9]{0,7}\\-[A-Z0-9]{0,4}\\-[A-Z0-9]{0,8}\\-[A-Z0-9.]{0,8}", fltline)){
						...
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "FLT LINE FORMAT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "FLT 포멧 입력 오류");
						}
						flag = false;
					}
					...
					if(Pattern.matches("[/.FLT.]{6}[0-9]{8}", flt_line[0])){
						...
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "FLIGHT DATE INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "입/출항일 입력 오류");
						}
						flag = false;
					}
					...
					if(Pattern.matches("[A-Z0-9]{2}[0-9]{3,5}", flt_line[1]) || Pattern.matches("[A-Z0-9]{2}[0-9]{3,4}[A-Z]{1}", flt_line[1])){
						...
						//20170529 최재준 추가
						//FLT CARR CODE 숫자만 입력 시 ERROR
						if(Pattern.matches("[0-9]{2}", flt_air)){

							map.put("FLT_AIRLINECODE", flt_air);
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "INVALID FLT CARR CODE");
								map.put("IKAMS_ERRMSG_KOR", "FLT CARR CODE 입력 오류"+flt_line[1].substring(0,2));
							}
							flag = false;
						}else{
							map.put("FLT_AIRLINECODE", flt_air);
						...
						if(flt_line[1].substring(2).trim().length() < 4){
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "INVALID FLT NO");
								map.put("IKAMS_ERRMSG_KOR", "FLT NO 입력 오류"+flt_line[1].substring(2));
							}
							flag = false;
						}
					}else{
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "FLIGHT INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "비행편명 입력 오류");
						}
						flag = false;
					}
					...
					if(Pattern.matches("[A-Z0-9]{4}", flt_line[2])){
						...
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "DECONSOL INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "세관 부호 코드 입력 오류");
						}
						flag = false;
					}
					...
					if (flt_line.length == 4 && Pattern.matches("[A-Z0-9]{0,8}", flt_line[3])) {
						...
						if(flt_line[4].replace(".", "").length() == 0){
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "WAREHOUSE CODE INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "WAREHOUSE CODE 입력 오류"+flt_line[4]);
							}
							flag = false;
						}else{
							map.put("FLT_LOCATION1", flt_line[3]);
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "FLT LINE MANDATORY");
						map.put("IKAMS_ERRMSG_KOR", "FLAG M or I 일 경우 편명 정보 입력 필수");
					}
					flag = false;
				}
			}catch(Exception e){
				e.printStackTrace();
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL TXT FLT LINE ERROR");
					map.put("IKAMS_ERRMSG_KOR", "편명 정보 입력 오류");
				}
				flag = false;
			}
			...
			try{
				...
				if(!dcsline.equals("")){
					...
					//관세청 4세대 DCS 포워더명 60 -> 100자리로 증가 2번 반복 가능
					String[] dcs_line = dcsline.split("/.DCS.");

					if(dcs_line.length >= 2){
						String dcs_line1 = CommonUtil.nullChk(dcs_line[1]);

						if (Pattern.matches("[A-Z0-9\\s\\-\\.]{1,60}", dcs_line1)) {
							if(dcs_line1.indexOf("(") !=-1 || dcs_line1.indexOf(")") !=-1){
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "DCS LINE INVALID CHARACTER INPUT ERROR");
									map.put("IKAMS_ERRMSG_KOR", "DCS LINE 입력 할 수 없는 문자 입력");
								}
								flag = false;
							}else{

								if (dcs_line1.replace(" ", "").length() == 0 || dcs_line1.replace(".", "").length() == 0|| dcs_line1.replace("-", "").length() == 0) {
									if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
										map.put("IKAMS_ERRMSG", "DCS INSERT ERROR");
										map.put("IKAMS_ERRMSG_KOR", "DECONSOL NAME 항목 입력 오류");
									}
									flag = false;
								}else{
									map.put("DECONSOL_NAME", dcs_line1.trim());
								}
							}
						}else{
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "DCS LINE ERROR");
								map.put("IKAMS_ERRMSG_KOR", "DECONSOL NAME 60자리 입력 오류");
							}
							flag = false;
						}

						if(dcs_line.length == 3){

							String dcs_line2 = CommonUtil.nullChk(dcs_line[2]);

							if (Pattern.matches("[A-Z0-9\\s\\-\\.]{1,40}", dcs_line2)) {
								if(dcs_line2.indexOf("(") !=-1 || dcs_line2.indexOf(")") !=-1){
									if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
										map.put("IKAMS_ERRMSG", "DCS LINE INVALID CHARACTER INPUT ERROR");
										map.put("IKAMS_ERRMSG_KOR", "DCS LINE 입력 할 수 없는 문자 입력");
									}
									...
									if (dcs_line2.replace(" ", "").length() == 0 || dcs_line2.replace(".", "").length() == 0|| dcs_line2.replace("-", "").length() == 0) {
										if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
											map.put("IKAMS_ERRMSG", "DCS INSERT ERROR");
											map.put("IKAMS_ERRMSG_KOR", "DECONSOL NAME  항목 입력 오류");
										}
										...
										map.put("DECONSOL_NAME", map.get("DECONSOL_NAME") + " " + dcs_line2.trim());
									}
								}
							}else{
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "DCS LINE ERROR");
									map.put("IKAMS_ERRMSG_KOR", "DECONSOL NAME 40자리 초과 항목 입력 오류");
								}
								flag = false;
							}
						}
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "DCS LINE MANDATORY");
						map.put("IKAMS_ERRMSG_KOR", "수입 건 DCS 정보 입력 필수");
					}
					flag = false;
				}
				...
				e.printStackTrace();
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL TEXT DCS LINE ERROR");
					map.put("IKAMS_ERRMSG_KOR", "TEXT DCS 라인 입력 오류");
				}
				flag = false;
			}
			...
			try{
					...
					if(snm_line.length >=2){
						...
						if(Pattern.matches("[A-Z0-9\\s\\-\\.]{1,60}", snm_name)){
							...
							if (snm_name.replace(" ", "").length() == 0 || snm_name.replace(".", "").length() == 0|| snm_name.replace("-", "").length() == 0) {
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "SNM INSERT ERROR");
									map.put("IKAMS_ERRMSG_KOR", "SNM NAME 송하인 이름 항목 입력 오류");
								}
								flag = false;
							}else{
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "SNM LINE ERROR");
								map.put("IKAMS_ERRMSG_KOR", "SNM LINE 60자 초과 항목 입력 오류");
							}
							flag = false;
						}
						...
						if(snm_line.length == 3){
							...
							if(Pattern.matches("[A-Z0-9\\s\\-\\.]{1,40}", snm_name2)){
								...
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "SNM LINE ERROR");
									map.put("IKAMS_ERRMSG_KOR", "SNM LINE 40자 초과 항목 입력 오류");
								}
								flag = false;
							}
						}
				...
				e.printStackTrace();
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL TEXT SNM LINE ERROR");
					map.put("IKAMS_ERRMSG_KOR", "TEXT SNM 라인 입력 오류");
				}
				flag = false;
			}
			...
			try{
				...
				if(!sccline.equals("")){
					if(Pattern.matches("[/]{1}(.SCC.)[A-Z0-9\\s\\-\\.]{5}", sccline)){
						...
						if (scc_code.replace(" ", "").length() == 0 || scc_code.replace(".", "").length() == 0|| scc_code.replace("-", "").length() == 0) {
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "SCC INSERT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "SCC CODE 송하인 식별자 코드 항목 입력 오류");
							}
							flag = false;
						}else{
						...
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "SCC LINE ERROR");
							map.put("IKAMS_ERRMSG_KOR", "SCC LINE 항목 입력 오류");
						}
						flag = false;
					}
				}
				...
				e.printStackTrace();
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL TEXT SCC LINE ERROR");
					map.put("IKAMS_ERRMSG_KOR", "TEXT SCC 라인 입력 오류");
				}
				flag = false;
			}
			...
			try{
					...
					if(sar_line.length >= 2){
						...
						if(Pattern.matches("[A-Z0-9\\s\\-\\.]{1,60}", sar_addr)){
							...
							if (sar_addr.replace(" ", "").length() == 0 || sar_addr.replace(".", "").length() == 0|| sar_addr.replace("-", "").length() == 0) {
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "SAR INSERT ERROR");
									map.put("IKAMS_ERRMSG_KOR", "SAR ADDRESS 송하인 주소 항목 입력 오류");
								}
								flag = false;
							}else{
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "SAR LINE ERROR");
								map.put("IKAMS_ERRMSG_KOR", "SAR LINE 60자리 항목 입력 오류");
							}
							flag = false;
						}
						...
						if(sar_line.length >=3){
							...
							if(sar_line.length ==4){
								...
								if(Pattern.matches("[A-Z0-9\\s\\-\\.]{1,30}", sar_addr3)){
									...
									if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
										map.put("IKAMS_ERRMSG", "SAR LINE ERROR");
										map.put("IKAMS_ERRMSG_KOR", "SAR LINE 30 자리 항목 입력 오류");
									}
									flag = false;
								}
							}
				...
				e.printStackTrace();
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL TEXT SAR LINE ERROR");
					map.put("IKAMS_ERRMSG_KOR", "TEXT SAR 라인 입력 오류");
				}
				flag = false;
			}
			...
			try{
				...
				if(!cnmline.equals("")){
					...
					if(cnm_line.length >= 2){
						...
						if(Pattern.matches("[A-Z0-9\\s\\-\\.]{1,60}", cnm_name)){
							...
							if (cnm_name.replace(" ", "").length() == 0 || cnm_name.replace(".", "").length() == 0|| cnm_name.replace("-", "").length() == 0) {
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "CNM INSERT ERROR");
									map.put("IKAMS_ERRMSG_KOR", "CNM NAME 수하인 이름 항목 입력 오류");
								}
								flag = false;
							}else{
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "CNM LINE ERROR");
								map.put("IKAMS_ERRMSG_KOR", "CNM LINE 60자 항목 입력 오류");
							}
							flag = false;
						}
						...
						if(cnm_line.length ==3){
							...
							if(Pattern.matches("[A-Z0-9\\s\\-\\.]{1,40}", cnm_name2)){
								...
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "CNM LINE ERROR");
									map.put("IKAMS_ERRMSG_KOR", "CNM LINE 40자 항목 입력 오류");
								}
								flag = false;
							}
					...
					e.printStackTrace();
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "FHL TEXT CNM LINE ERROR");
						map.put("IKAMS_ERRMSG_KOR", "TEXT CNM 라인 입력 오류");
					}
					flag = false;
			}
			...
			try{
				...
				if(!cccline.equals("")){
					if(Pattern.matches("[/]{1}(.CCC.)[A-Z0-9\\s\\-\\.]{5}", cccline)){
						...
						if (ccc_code.replace(" ", "").length() == 0 || ccc_code.replace(".", "").length() == 0|| ccc_code.replace("-", "").length() == 0) {
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "CCC INSERT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "CCC CODE 수하인 도시코드 항목 입력 오류");
							}
							flag = false;
						}else{
						...
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "CCC LINE ERROR");
							map.put("IKAMS_ERRMSG_KOR", "CCC LINE 항목 입력 오류");
						}
						flag = false;
					}
				}
				...
				e.printStackTrace();
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL TEXT CCC LINE ERROR");
					map.put("IKAMS_ERRMSG_KOR", "TEXT CCC 라인 입력 오류");
				}
				flag = false;
			}
			...
			try{
					...
					if(car_line.length >= 2){
						...
						if(Pattern.matches("[A-Z0-9\\s\\-\\.]{1,60}", car_addr)){
							...
							if (car_addr.replace(" ", "").length() == 0 || car_addr.replace(".", "").length() == 0|| car_addr.replace("-", "").length() == 0) {
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "CAR INSERT ERROR");
									map.put("IKAMS_ERRMSG_KOR", "CAR ADDRESS 송하인 주소 항목 입력 오류");
								}
								flag = false;
							}else{
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "CAR LINE ERROR");
								map.put("IKAMS_ERRMSG_KOR", "CAR LINE 60자 항목 입력 오류");
							}
							flag = false;
						}
						...
						if(car_line.length >=3){
							...
							if(car_line.length ==4){
								...
								if(Pattern.matches("[A-Z0-9\\s\\-\\.]{1,30}", car_addr3)){
									...
									if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
										map.put("IKAMS_ERRMSG", "CAR LINE ERROR");
										map.put("IKAMS_ERRMSG_KOR", "CAR LINE 30자 항목 입력 오류");
									}
									flag = false;
								}
							}
				...
				e.printStackTrace();
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL TEXT CAR LINE ERROR");
					map.put("IKAMS_ERRMSG_KOR", "TEXT CAR 라인 입력 오류");
				}
				flag = false;
			}
			...
			if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
				map.put("IKAMS_ERRMSG", "FHL TXT LINE ERROR");
				map.put("IKAMS_ERRMSG_KOR", "품목 편명 정보 입력 오류");
			}
			flag = false;
		}
		return flag;
	...
	public boolean HTSCheck(HashMap<String, Object> map) {
		...
		boolean flag = true;
		try{
			//관세청 4세대 1~10 자리 이나 수출 신고시 6자리 이상 필수
			//CIMP 6~18 자리 확인 필요
			if(!Pattern.matches("(HTS(/[A-Z0-9]{6,18}[\r\n]*){1,9})?", map.get("HTS_LINE").toString())){
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL HTS LINE ERROR");
					map.put("IKAMS_ERRMSG_KOR", "HTS LINE 에러");
				}
				flag = false;
			}else{
	...
	public boolean OCICheck(HashMap<String, Object> map) throws Exception{
		...
		boolean flag = true;
		int result = 0;
		unipassAPI api = new unipassAPI();
		try{
			String oci_chk = map.get("OCI_LINE_CHK_I").toString().replace("OCI", "");
			...
			for(int i=0; i<list.size();i++){
				...
				if(Pattern.matches("[KR]{2}", oci_line[1])){
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "OCI ISO CODE INPUT ERROR");
						map.put("IKAMS_ERRMSG_KOR", "OCI ISO CODE ERROR");
					}
					flag = false;
				}
				if(Pattern.matches("EXP|IMP", oci_line[2])){
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "OCI TYPE INPUT ERROR");
						map.put("IKAMS_ERRMSG_KOR", "OCI TYPE ERROR");
					}
					flag = false;
				}
				//20161115 FLAG 'X'건 추가
				if(Pattern.matches("[MIAEX]{1}", oci_line[3])){
					map.put("OCI_FLAG", oci_line[3].toString());
				}else{
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "OCI FLAG INPUT ERROR");
						map.put("IKAMS_ERRMSG_KOR", "OCI FLAG ERROR");
					}
					flag = false;
				}


				//OCI/KR/EXP/M/E001-260494038432-A-1-CT
				//EPN NO 20자리로 증가, 동시포장개수 10자리 변경, 선적포장종류코드 2 자리 추가
				if(CommonUtil.nullChk(map.get("OCI_TYPE")).equals("EXP")){
					...
					if(OCILine1.length == 2){
						if(Pattern.matches("[ER]{1}[0-9]{3}", OCILine1[0])){
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI CARGO TYPE INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "화물타입 입력 오류");
							}
							flag = false;
						}
						//관세청 4세대 EPN NO 20자리로 증가
						//기존은 15자리까지 가능
						if(Pattern.matches("[0-9A-Z]{3,19}",OCILine1[1])){
							map.put("OCI_EPN_NO",CommonUtil.nullChk(OCILine1[1],""));
						}else{
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI EPN TYPE NUMBER INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "수출 신고 번호 입력 오류");
							}
							flag = false;
						}
						...
						if(Pattern.matches("[A-Z]{0,1}",OCILine1[2])){
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI SPACK CODE INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "동시포장코드 입력 오류");
							}
							flag = false;
						}
						map.put("OCI_EPN_SPACK_PC","0");
						...
						if(OCILine1.length <= 1){
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI EPN NUMBER INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "수출 신고 번호 입력 오류");
							}
							flag = false;
						}else{
						...
						if(OCILine1[3].trim().equals(null)||OCILine1[3].trim().length()==0){
							...
							if(Pattern.matches("[0-9.]{0,4}",OCILine1[3])){
								...
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "OCI SPACK PC INPUT ERROR");
									map.put("IKAMS_ERRMSG_KOR", "동시포장 갯수 입력 오류");
								}
								flag = false;
							}
						}
							...
							if(OCILine1.length > 4){
								if(Pattern.matches("[A-Z0-9]{2}",OCILine1[4])){
									...
									if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
										map.put("IKAMS_ERRMSG", "OCI SHIPPER TARE CODE INPUT ERROR");
										map.put("IKAMS_ERRMSG_KOR", "선적포장코드 입력 오류");
									}
									flag = false;
								}
							}
							if(OCILine1.length >= 6){
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "OCI INPUT ERROR");
									map.put("IKAMS_ERRMSG_KOR", "OCI LINE 입력 횟수 초과");
								}
								flag = false;
							}
						}
					...
					if(SEQ_Check.indexOf(","+CommonUtil.nullChk(OCILine1[0].substring(1).toString(),"")+",") !=-1){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "OCI EPN SEQ OVERLAP ERROR");
							map.put("IKAMS_ERRMSG_KOR", "EPN SEQ 중복 오류");
						}
						flag = false;
					}else{
						...
						if(EPN_Check.indexOf(","+CommonUtil.nullChk(OCILine1[1],"")+",") != -1){
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI EPN NO OVERLAP ERROR");
								map.put("IKAMS_ERRMSG_KOR", "EPN NO 중복 오류");
							}
							flag = false;
						}else{
					...
					//S001-1-212.0-Y-1
					//EPN 수량 8 ->10자리로 변경, EPN 중량 11 -> 자리수 변경(n[..7]p->n[..17]p)(정수(13) + 소수점 + 소수이하(3))
					if(OCILine2.length==3){
						if(Pattern.matches("[S]{1}[0-9]{3}", OCILine2[0])){

						}else{
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI EVEN CARGO TYPE INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "화물 입력 오류");
							}
							flag = false;
						}
						if(Pattern.matches("[0-9]{1,10}",OCILine2[1])){
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI EPN PC INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "수출신고 수량 입력 오류");
							}
							flag = false;
						}
						if(Pattern.matches("[0-9]{1,13}([.]{1}[0-9]{1,3})?",OCILine2[2])){
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI EPN WT INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "수출신고 중량 입력 오류");
							}
							flag = false;
						}
						...
						if(Pattern.matches("[A-Z]{0,1}",OCILine2[3])){
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI SPLIT CODE INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "분할 여부 코드  입력 오류");
							}
							flag = false;
						}
						...
						if(Pattern.matches("[0-9]{0,1}",OCILine2[4])){
							...
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI SPLIT SEQ INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "분할 차수 코드  입력 오류");
							}
							flag = false;
						}
						...
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "OCI INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "OCI LINE2 입력 회수 초과");
						}
						flag = false;
					}
					//20161206 최재준 추가
					...
					if(SEQ_Check2.indexOf(","+CommonUtil.nullChk(OCILine2[0].substring(1).toString(),"")+",") !=-1){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "OCI EPN SEQ OVERLAP ERROR");
							map.put("IKAMS_ERRMSG_KOR", "EPN SEQ 2번째 라인 중복 오류");
						}
						flag = false;
					}else{
					...
					if(map.get("OCI_CHECK").equals("P")){
						...
						if((map.get("CARR_CODE").equals("KE") || map.get("CARR_CODE").equals("LJ")) && !map.get("OCI_EPN_NO").equals("NCV") && !map.get("OCI_CARGO_TYPE").equals("R")){
							map = api.setFulfillepn(map);
							map.get("APIflag");
						}
						if(CommonUtil.nullChk(map.get("OCI_EPN_NO")).equals("NCV")){
							map.put("EPN_ERR_YN", "");
				        	map.put("EPN_CHK_MSG", "");
						}
						if(map.get("OCI_CARGO_TYPE").equals("R")){
							map.put("EPN_ERR_YN", "R");
				        	map.put("EPN_CHK_MSG", "환적심사 생략");
						}
						result = dbinserttrace.ikamsExpEpnInsert(map);
						logger.info(" === IKAMS EXP EPN INSERT === ");
					}

					//OCI/KR/IMP/I/I0001-GD
					//HSN 3->4자리 변경
				}else if(CommonUtil.nullChk(map.get("OCI_TYPE")).equals("IMP")){
					String org_code = "ICN,SEL,PUS,GMP";

					//20170213 FLAG 'X'건 추가
					if(oci_line.length <= 5){
						...
						||(map.get("OCI_FLAG").equals("X") && Pattern.matches("[ITP]{1}[0-9]{3}\\-[A-Z]{0,2}", oci_line[4]))
						|| (map.get("OCI_FLAG").equals("X") && Pattern.matches("[ITP]{1}[0-9]{4}\\-[A-Z]{0,2}", oci_line[4]))){
							...
							if(CommonUtil.nullChk(map.get("OCI_HSN")).equals("0000")){
								...
								if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
									map.put("IKAMS_ERRMSG", "OCI IMP HSN INPUT ERROR");
									map.put("IKAMS_ERRMSG_KOR", "수입 OCI HNS 항목 입력 오류");
								}
								flag = false;
							}
						}else{
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "OCI IMP INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "수입 OCI 항목 입력 오류");
							}
							flag = false;
						}
					}else{
					...
					if(CommonUtil.nullChk(map.get("OCI_CARGO_TYPE")).equals("T")){
						...
						if(org_code.indexOf(map.get("M_ORG").toString()) !=-1){
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "IMP TRANS FHL ORG INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "수입 환적 FHL ORG 입력 오류");
							}
							flag = false;
						}
					}else{
						...
						if(org_code.indexOf(map.get("M_ORG").toString()) !=-1 || org_code.indexOf(map.get("H_ORG").toString()) !=-1){
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "IMP FHL ORG INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "수입 FHL ORG 입력 오류");
							}
							flag = false;
						}
					}
				...
				if(map.get("OCI_CHECK").equals("C") && (map.get("OCI_FLAG").equals("I") || map.get("OCI_FLAG").equals("X"))){
			...
			//20191204 최재준 추가
			//관세청 수출 epn no 체크 api(KE/LJ만)
			if(CommonUtil.nullChk(map.get("APIflag")).equals("false")){
			...
			if(Pattern.matches("(\\/(KR)\\/(EXP||IMP)\\/[A-Z]{1}\\/[A-Z0-9\\s\\-\\.]{1,35}\r\n)+", oci_chk)){
				...
				logger.info(" === OCI Validation Check Success === ");
			}else{

				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "FHL OCI LINE INPUT ERROR");
					map.put("IKAMS_ERRMSG_KOR", "");
				}
				flag = false;
			}
			...
			e.printStackTrace();
			if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
				map.put("IKAMS_ERRMSG", "FHL OCI LINE ERROR");
				map.put("IKAMS_ERRMSG_KOR", "");
			}
			flag = false;
		}
	...
	public boolean OCICheck_Origin2(HashMap<String, Object> map) throws Exception{
		...
		try{
			...
			for(int i=0; i<oci_etc_line.length; i++) {//oci_etc_line[i]를 /(slant) 별로 쪼개기 oci는 slant 4개 필수 항목
				...
				if(CommonUtil.nullChk(map.get("OCI_FLAG")).equals("I") || CommonUtil.nullChk(map.get("OCI_FLAG")).equals("X")){
...
//				}else{
//					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
//						map.put("IKAMS_ERRMSG", "CN OCI SHP LINE INPUT ERROR");
//						map.put("IKAMS_ERRMSG_KOR", "");
//					}
//					flag = false;
//				}
			...
			e.printStackTrace();
			if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
				map.put("IKAMS_ERRMSG", "CN OCI SHP EXCEPTION");
				map.put("IKAMS_ERRMSG_KOR", "");
			}
			flag = false;
		}
...
//			if(Pattern.matches("((\\/[A-Z]{2}\\/(CNE)\\/[A-Z]{1,2}\\/[A-Z0-9\\s\\-\\.]{1,35})\r\n)+", oci_cne_chk)){
...
//			}else{
//				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
//					map.put("IKAMS_ERRMSG", "CN OCI CNE LINE INPUT ERROR");
//					map.put("IKAMS_ERRMSG_KOR", "");
//				}
//				flag = false;
//			}
			...
			e.printStackTrace();
			if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
				map.put("IKAMS_ERRMSG", "CN OCI CNE EXCEPTION");
				map.put("IKAMS_ERRMSG_KOR", "");
			}
			flag = false;
		}
...
//			if(Pattern.matches("((\\/[A-Z]{2}\\/(NFY)\\/[A-Z]{1,2}\\/[A-Z0-9\\s\\-\\.]{1,35})\r\n)+", oci_nfy_chk)){
...
//			}else{
//				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
//					map.put("IKAMS_ERRMSG", "CN OCI NFY LINE INPUT ERROR");
//					map.put("IKAMS_ERRMSG_KOR", "");
//				}
//				flag = false;
//			}
			...
			e.printStackTrace();
			if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
				map.put("IKAMS_ERRMSG", "CN OCI NFY EXCEPTION");
				map.put("IKAMS_ERRMSG_KOR", "");
			}
			flag = false;
		}
	...
	public boolean SHPCheck(HashMap<String, Object> map) {
		...
		if(shp_check.length == 4){
			...
			try{
				if(fwrdname.startsWith("SHP/") && Pattern.matches("[SHP]{3}\\/[A-Z0-9\\s\\-\\.]{1,35}", fwrdname.trim())){
					...
					if(shpname.replace(" ","").length()==0){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "SHP NAME INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "송하인 회사명 입력 오류");
						}
						flag = false;
					}else{
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "SHP NAME LINE ERROR");
						map.put("IKAMS_ERRMSG_KOR", "송하인 회사명 오류");
					}
					flag = false;
				}
				...
				if(Pattern.matches("[/]{1}[A-Z0-9\\s\\-\\.]{1,35}", fwrdaddr.trim())){
					...
					if(shpaddress.replace(" ", "").length()==0){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "SHP ADDRESS INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "송하인 주소 항목 입력 오류");
						}
						flag = false;
					}else{
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "SHP ADDRESS LINE ERROR");
						map.put("IKAMS_ERRMSG_KOR", "송하인 주소 항목 오류");
					}
					flag = false;
				}
				...
				if(Pattern.matches("^/[A-Z0-9\\s\\-\\.]{1,17}(\\/[A-Z0-9\\s\\-\\.]{1,9})?", fwrdloc.trim())){
					...
					if(shploc[1].replace(" ", "").length()==0){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "SHP PLACE INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "송하인 장소 항목 오류");
						}
						flag = false;
					}else{
					...
					if(shploc.length==3){
						if(shploc[2].replace(" ", "").length()==0){
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "SHP STATEPROVINCE INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "송하인 주코드 항목 오류");
							}
							flag = false;
						}else{
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "SHP LOCATION LINE ERROR");
						map.put("IKAMS_ERRMSG_KOR", "송하인 위치 항목 오류");
					}
					flag = false;
				}
				...
				if(Pattern.matches("^[/]{1}[A-Z]{2}(([/]{1}[A-Z0-9\\s\\-\\.]{1,9})?(([/]{1}[A-Z0-9]{1,3}[/]{1}[A-Z0-9]{1,25})?){1,2}$|([/]{1}([/]{1}[A-Z0-9]{1,3}[/]{1}[A-Z0-9]{1,25}){1,2})?$)", fwrdcode.trim())){
					...
					if(coded.length==2){
						...
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "SHP LOCATION CODE FORMAT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "송하인 국가코드 입력 오류");
						}
						flag = false;
					}
				}else{
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "SHP CODE LOCATION INPUT ERROR");
						map.put("IKAMS_ERRMSG_KOR", "송하인 정보 입력 오류");
					}
					flag = false;
				}
				...
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "SHP LINE CHECK EXCEPTION");
					map.put("IKAMS_ERRMSG_KOR", "송하인 라인 입력 EXCEPTION");
				}
				flag = false;
			}
			...
			if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
				map.put("IKAMS_ERRMSG", "SHP LINE INPUT ERROR");
				map.put("IKAMS_ERRMSG_KOR", "송하인 라인 입력 오류");
			}
			flag = false;
		}
		return flag;
			...
			try{
				if(cnedname.startsWith("CNE/") && Pattern.matches("[CNE]{3}\\/[A-Z0-9\\s\\-\\.]{1,35}", cnedname.trim())){
					...
					if(cnename.replace(" ", "").length()==0){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "CNE NAME INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "수하인 회사명 입력 오류");
						}
						flag = false;
					}else{
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "CNE NAME LINE ERROR");
						map.put("IKAMS_ERRMSG_KOR", "수하인 회사명 오류");
					}
					flag = false;
				}
				...
				if(Pattern.matches("[/]{1}[A-Z0-9\\s\\-\\.]{1,35}", cnedaddr.trim())){
					...
					if(cneaddress.replace(" ", "").length()==0){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "CNE ADDRESS INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "수하인 주소 항목 입력 오류");
						}
						flag = false;
					}else{
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "CNE ADDRESS LINE ERROR");
						map.put("IKAMS_ERRMSG_KOR", "수하인 주소 항목 오류");
					}
					flag = false;
				}
				...
				if(Pattern.matches("^/[A-Z0-9\\s\\-\\.]{1,17}(\\/[A-Z0-9\\s\\-\\.]{1,9})?", cnedloc.trim())){
					...
					if(cneloc[1].replace(" ", "").length()==0){
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "CNE PLACE INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "수하인 장소 항목 오류");
						}
						flag = false;
					...
					if(cneloc.length==3){
						if(cneloc[2].replace(" ", "").length()==0){
							if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
								map.put("IKAMS_ERRMSG", "CNE STATEPROVINCE INPUT ERROR");
								map.put("IKAMS_ERRMSG_KOR", "수하인 주코드 항목 오류");
							}
							flag = false;
						}else{
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "CNE LOCATION INPUT ERROR");
						map.put("IKAMS_ERRMSG_KOR", "수하인 위치 항목 오류");
					}
					flag = false;
				}
				...
				if(Pattern.matches("^[/]{1}[A-Z]{2}(([/]{1}[A-Z0-9\\s\\-\\.]{1,9})?(([/]{1}[A-Z0-9]{1,3}[/]{1}[A-Z0-9]{1,25})?){1,2}$|([/]{1}([/]{1}[A-Z0-9]{1,3}[/]{1}[A-Z0-9]{1,25}){1,2})?$)", cnedcode.trim())){
					...
					if(cne_coded.length==2){
						...
						if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
							map.put("IKAMS_ERRMSG", "CNE LOCATION FORMAT INPUT ERROR");
							map.put("IKAMS_ERRMSG_KOR", "수하인 국가코드 항목 입력 오류");
						}
						flag = false;
					}
					...
					//Business Check
					//적하 용일 경우 비지니스 스펙 제외
					/*if(!(CommonUtil.nullChk(map.get("OCI_TYPE")).equals("EXP") && CommonUtil.nullChk(map.get("OCI_FLAG")).equals("I"))){
						flag = bizcheck.BIZcheck(map);
					}*/
				}else{
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "CNE CODED LOCATION INPUT ERROR");
						map.put("IKAMS_ERRMSG_KOR", "수하인 정보 입력 오류");
					}
					flag = false;
				}
				...
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "CNE LINE CHECK EXCEPTION");
					map.put("IKAMS_ERRMSG_KOR", "수하인 항목 입력 오류");
				}
				flag = false;
			}
		}else{
			if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
				map.put("IKAMS_ERRMSG", "CNE LINE LINE ERROR");
				map.put("IKAMS_ERRMSG_KOR", "수하인 항목 입력 오류");
			}
			flag = false;
		...
		if(cvd_line.length ==6){
			...
			try{
				if(Pattern.matches("[CVD]{3}\\/[A-Z]{3}\\/[PP|PC|CC|CP]{2}\\/([NVD]{3}|[0-9.]{1,12})\\/([NCV]{3}|[0-9.]{1,12})\\/([XXX]{3}|[0-9.]{1,11})", cvdline)){
					...
					if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
						map.put("IKAMS_ERRMSG", "CVD LINE CHECK ERROR");
						map.put("IKAMS_ERRMSG_KOR", "CVD 입력 오류");
					}
					flag = false;
				}
			}catch(Exception ee){
				if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
					map.put("IKAMS_ERRMSG", "CVD LINE CHECK EXCEPTION");
					map.put("IKAMS_ERRMSG_KOR", "형식이 맞지 않은 운임정보가 입력 되었습니다.");
				}
				flag = false;
			}
		}else{
			if(CommonUtil.nullChk(map.get("IKAMS_ERRMSG"),"").equals("")){
				map.put("IKAMS_ERRMSG", "CVD LINE LINE ERROR");
				map.put("IKAMS_ERRMSG_KOR", "운임정보 입력 오류");
			}
			flag = false;
		}
		return flag;
